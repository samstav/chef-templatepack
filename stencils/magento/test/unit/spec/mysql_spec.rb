...
describe '|{.Cookbook.Name}|::mysql' do
      ...
      context "on #{platform.capitalize} #{version}" do
        ...
        context 'for mysql_master' do
          cached(:chef_run) do
            ChefSpec::ServerRunner.new(platform: platform, version: version, log_level: ::LOG_LEVEL) do |node, server|
              node_resources(node)
            end.converge('|{.Cookbook.Name}|::mysql_master')
          end
        ...
        context 'for mysql_slave' do
          cached(:chef_run) do
            ChefSpec::ServerRunner.new(platform: platform, version: version, log_level: ::LOG_LEVEL) do |node, server|
              ...
              node_resources(node)
            end.converge('|{.Cookbook.Name}|::mysql_slave')
          end
        ...
        context 'for mysql_holland' do
          cached(:chef_run) do
            ChefSpec::ServerRunner.new(platform: platform, version: version, log_level: ::LOG_LEVEL) do |node, server|
              node_resources(node)
            end.converge('|{.Cookbook.Name}|::mysql_holland')
          end
        ...
        context 'for mysql_add_drive' do
          cached(:chef_run) do
            ChefSpec::ServerRunner.new(platform: platform, version: version, log_level: ::LOG_LEVEL) do |node, server|
              node_resources(node)
            end.converge('|{.Cookbook.Name}|::mysql_add_drive')
          end
