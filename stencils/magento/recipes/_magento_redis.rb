...
session_master_name, session_master_ip, session_master_port = MagentoUtil.best_redis_session_master(node)
if session_master_name && session_master_ip && session_master_port
  ...
  xml_edit 'enable redis in ./app/etc/modules/Cm_RedisSession.xml' do
    path "#{node['|{.Cookbook.Name}|']['web']['dir']}/app/etc/modules/Cm_RedisSession.xml"
  ...
  xml_edit 'set session_store to db in ./app/etc/local.xml' do
    path "#{node['|{.Cookbook.Name}|']['web']['dir']}/app/etc/local.xml"
  ...
  <password>#{MagentoUtil.redis_session_password(node)}</password>
  <timeout>2.5</timeout>
  ...
  Chef::Log.warn('|{.Cookbook.Name}|::_magento_redis could not locate a master redis session node')
end

object_master_name, object_master_ip, object_master_port = MagentoUtil.best_redis_object_master(node)
if object_master_name && object_master_ip && object_master_port
  redis_object_fragment = "<cache>
        ...
        <database>0</database>
        <password>#{MagentoUtil.redis_object_password(node)}</password>
  ...
  Chef::Log.warn('|{.Cookbook.Name}|::_magento_redis could not locate an appropriate redis object cache node')
end

# discovery function is edition (community/enterprise) aware, won't find an instance when not using enterprise
page_master_name, page_master_ip, page_master_port = MagentoUtil.best_redis_page_master(node)
if page_master_name && page_master_ip && page_master_port
  ...
  <backend_options>
    ...
    <database>1</database>
    <password>#{MagentoUtil.redis_page_password(node)}</password>
    <force_standalone>0</force_standalone>  <!-- 0 for phpredis, 1 for standalone PHP -->
    <connect_retries>1</connect_retries>    <!-- Reduces errors due to random connection failures -->
    <automatic_cleaning_factor>0</automatic_cleaning_factor> <!-- Disabled by default -->
  ...
  Chef::Log.warn('|{.Cookbook.Name}|::_magento_redis could not locate an appropriate redis page cache node')
end
